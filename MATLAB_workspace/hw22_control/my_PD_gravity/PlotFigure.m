...
figure('name', 'Scheme F: PD control with gravity compensation');

if c == 0,
    % joint angle
    fig1 = subplot(2, 2, 1);
    plot(time, q_d(:, 1), '--', time, q(:, 1)), grid on;
    legend(fig1, {'Ref', 'Rel'}, 'Location', 'best')
    ...
    legend(fig2, {'Ref', 'Rel'}, 'Location', 'best')
    %axis([0 t_d -4000 4000]);
    set(gca, 'fontname', 'Times', 'fontsize', 12, 'fontweight', 'normal');
    xlabel('[s]');
    ylabel('[rad]');
    ...
    legend(fig3, {'Tau1', 'Tau2'}, 'Location', 'best')
    %axis([0 t_d -4000 4000]);
    set(gca, 'fontname', 'Times', 'fontsize', 12, 'fontweight', 'normal');
    xlabel('[s]');
    ylabel('[Nm]');
