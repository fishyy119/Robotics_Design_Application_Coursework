...
syms fx_p1 fy_p1 fz_p1 k_spring k_damper real%长方体其中一个顶点受到的力f，模拟地面碰撞的弹性系数和阻尼系数
...
Jac_p1 = jacobian(p1,q);%p1点的雅可比矩阵
Tau = Jac_p1'*[fx_p1 fy_p1 fz_p1]';%用雅可比矩阵，把力f匹配到每个轴上
SE = 1/2*k_spring*p1(3)*p1(3);%模拟地面碰撞弹簧的弹性势能，用于能量校核

for ii=1:6
    L(ii) = Ldd_qd_dt(ii) - Ld_q(ii)-Tau(ii);%增加了力矩Tau
...
fprintf(fid, 'function zdot=rhs(t,z,flag,m,xG,yG,zG,I1,g,lCuboid,wCuboid,hCuboid,k_spring)   \n\n');
...
fprintf(fid,'if z_p1<0\n');
fprintf(fid,'fx_p1 = 0;\n');
fprintf(fid,'fy_p1 = 0;\n');
fprintf(fid,'fz_p1 = -k_spring * z_p1;\n');
fprintf(fid,'else \n');
fprintf(fid,'fx_p1 = 0;\n');
fprintf(fid,'fy_p1 = 0;\n');
fprintf(fid,'fz_p1 = 0;\n');
...
fprintf(fid, 'function [KE, PE, SE]=energy(t,z,m,xG,yG,zG,I1,g,lCuboid,wCuboid,hCuboid,k_spring)   \n\n');
