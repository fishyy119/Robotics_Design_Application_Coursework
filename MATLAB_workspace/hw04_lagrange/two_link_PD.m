...
for k = 1:length(tspan)
    ...
    if t < t1
        ...
        [y, yd, ydd] = TSpline(-1, 0, 0, -0.9, t1/2, -0.75, 0, t1, t);
    else
        x = 0.25 * sin(w*(t - t1));
        xd = 0.25 * w * cos(w*(t - t1));
        xdd = -0.25 * w^2 * sin(w*(t - t1));
        y = -0.5 - 0.25 * cos(w*(t - t1));
        yd = 0.25 * w * sin(w*(t - t1));
        ydd = 0.25 * w^2 * cos(w*(t - t1));
    end
    x_traj(k) = x;
    y_traj(k) = y;
...
for i = 1:length(t)
    ...
    elapsed = toc;
    target_time = t(i);
    if elapsed < target_time, pause(target_time-elapsed); end
    % 写入视频
    frame = getframe(gcf);
...
q2 = acos(max(min(c2, 1), -1)); % 防止超出[-1,1]
