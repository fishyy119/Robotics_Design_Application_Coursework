...
x_tip_dot = expand(v_F(1));
y_tip = expand(r_O_F(2));
y_tip_dot = expand(v_F(2));
y_tip_min = -(l1 + l2);
PEg = expand(m1*g*(r_O_G1(2) - y_tip_min)+m2*g*(r_O_G2(2) - y_tip_min));
%% 写入自动生成文件
this_dir = fileparts(mfilename('fullpath'));
write_rhs_file(fullfile(this_dir, 'rhs_planar_contact.m'), M, RHS, x_tip_dot, y_tip, y_tip_dot);
write_energy_file(fullfile(this_dir, 'energy_planar_contact.m'), KE, PEg, x_tip_dot, y_tip, y_tip_dot);

disp('已生成 rhs_planar_contact.m 和 energy_planar_contact.m');
end

function write_rhs_file(filename, M, RHS, x_tip_dot, y_tip, y_tip_dot)
% 写入状态导数函数文件。
...
fprintf(fid, 'function zdot = rhs_planar_contact(~, z, m1, m2, l1, l2, a1, a2, J1, J2, g, ground_y, k_ground, c_ground, mu_ground, v_smooth)\n');
...
fprintf(fid, 'E_diss_damping = z(5);\n');
fprintf(fid, 'E_diss_friction = z(6);\n\n');
fprintf(fid, '%% 接触量\n');
fprintf(fid, 'tip_x_dot = %s;\n', char(x_tip_dot));
fprintf(fid, 'tip_y = %s;\n', char(y_tip));
...
fprintf(fid, 'Fx = 0;\n');
fprintf(fid, 'Fy = 0;\n');
fprintf(fid, 'damping_dissipation_rate = 0;\n');
fprintf(fid, 'friction_dissipation_rate = 0;\n');
fprintf(fid, 'if penetration > 0\n');
fprintf(fid, '    Fy = k_ground * penetration + c_ground * approach_speed;\n');
fprintf(fid, '    Fx = -mu_ground * Fy * tanh(tip_x_dot / max(v_smooth, 1e-6));\n');
fprintf(fid, '    damping_dissipation_rate = c_ground * approach_speed^2;\n');
fprintf(fid, '    friction_dissipation_rate = -Fx * tip_x_dot;\n');
fprintf(fid, 'end\n\n');
fprintf(fid, '%% 外加关节力矩\n');
fprintf(fid, 'T1 = 0;\n');
fprintf(fid, 'T2 = 0;\n\n');
...
fprintf(fid, 'zdot = [u1; qdd(1); u2; qdd(2); damping_dissipation_rate; friction_dissipation_rate];\n');
end

function write_energy_file(filename, KE, PEg, x_tip_dot, y_tip, y_tip_dot)
% 写入能量诊断函数文件。
...
fprintf(fid, 'function [KE, PEg, PEc, Pdiss_damping, Pdiss_friction, Fy, penetration, tip_y, tip_y_dot] = energy_planar_contact(~, z, m1, m2, l1, l2, a1, a2, J1, J2, g, ground_y, k_ground, c_ground, mu_ground, v_smooth)\n');
fprintf(fid, '%% 本文件由 derive_planar_contact.m 自动生成。\n');
fprintf(fid, '%% 如需修改推导过程，请编辑生成脚本而非直接手工修改本文件。\n\n');
fprintf(fid, '%% 状态量\n');
fprintf(fid, 'q1 = z(1); u1 = z(2);\n');
...
fprintf(fid, 'approach_speed = max(-tip_y_dot, 0);\n');
fprintf(fid, 'Fy = 0;\n');
fprintf(fid, 'Pdiss_damping = 0;\n');
fprintf(fid, 'Pdiss_friction = 0;\n');
fprintf(fid, 'if penetration > 0\n');
fprintf(fid, '    Fy = k_ground * penetration + c_ground * approach_speed;\n');
fprintf(fid, '    Fx = -mu_ground * Fy * tanh(tip_x_dot / max(v_smooth, 1e-6));\n');
fprintf(fid, '    Pdiss_damping = c_ground * approach_speed^2;\n');
fprintf(fid, '    Pdiss_friction = -Fx * tip_x_dot;\n');
fprintf(fid, 'end\n');
