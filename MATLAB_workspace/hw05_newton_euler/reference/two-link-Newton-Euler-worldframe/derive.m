...
f2 = m2*g*k + m2*a_G2;
 tau2 = simple(I2_w*al2 + cross(om2,I2_w*om2) + cross(r_E_G2,f2));
 %tau2 = simple(I2_w*al2  + cross(r_E_G2,f2));
f1 = m1*g*k + m1*a_G1 + f2;
 tau1 = simple(I1_w*al1 + cross(om1,I1_w*om1) + cross(r_O_G1,f1) + tau2 - cross(r_O_E-r_O_G1,-f2));
%tau1 = simple(I1_w*al1 + cross(r_O_G1,f1) + tau2 - cross(r_O_E-r_O_G1,-f2));
%%% The k component has the equations of motion %%%
eqn1 = collect(simple(T1-dot(tau1,Z1)),[ud1 ud2]);
eqn2 = collect(simple(T2-dot(tau2,X2)),[ud1 ud2]);
