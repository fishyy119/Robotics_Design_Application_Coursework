function zdot = rhs_planar_contact(~, z, m1, m2, l1, l2, a1, a2, J1, J2, g, ground_y, k_ground, c_ground, mu_ground, v_smooth)
...
Fx = 0;
Fy = 0;
damping_dissipation_rate = 0;
friction_dissipation_rate = 0;
if penetration > 0
    Fy = k_ground * penetration + c_ground * approach_speed;
    Fx = -mu_ground * Fy * tanh(tip_x_dot / max(v_smooth, 1e-6));
    damping_dissipation_rate = c_ground * approach_speed^2;
    friction_dissipation_rate = -Fx * tip_x_dot;
end
...
zdot = [u1; qdd(1); u2; qdd(2); damping_dissipation_rate; friction_dissipation_rate];
